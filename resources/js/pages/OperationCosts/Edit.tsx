...
interface Props { item: OperationCost }

export default function OperationCostsEdit({ item }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        operation_name: item.operation_name, operation_type: item.operation_type,
        country_code: item.country_code, min_cost: String(item.min_cost), max_cost: String(item.max_cost),
        is_active: item.is_active,
    })
    function handleSubmit(e: FormEvent) { e.preventDefault(); put(`/admin/operation-costs/${item.id}`) }
    return (
        <AdminLayout title="Edit Operation Cost">
            <div className="mx-auto" style={{ maxWidth: 640 }}>
                <ResourceForm title={`Edit: ${item.operation_name}`} onSubmit={handleSubmit} processing={processing} onCancel={() => window.history.back()}>
                    <div className="row g-3">
                        ...
                        <div className="col-md-4"><FormField label="Country" required error={errors.country_code}><CountrySelect value={data.country_code} onChange={(v) => setData('country_code', v)} error={errors.country_code} required /></FormField></div>
