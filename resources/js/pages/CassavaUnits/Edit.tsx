...
import type { CassavaUnit } from '../../types'

interface Props { item: CassavaUnit }

export default function CassavaUnitsEdit({ item }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        label: item.label, unit_weight: String(item.unit_weight),
        sort_order: String(item.sort_order ?? ''), description: item.description ?? '', is_active: item.is_active,
    })
    function handleSubmit(e: FormEvent) { e.preventDefault(); put(`/admin/cassava-units/${item.id}`) }
    return (
        <AdminLayout title="Edit Cassava Unit">
            <div className="mx-auto" style={{ maxWidth: 600 }}>
                <ResourceForm title={`Edit: ${item.label}`} onSubmit={handleSubmit} processing={processing} onCancel={() => window.history.back()}>
