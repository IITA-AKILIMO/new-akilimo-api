...
interface Props { item: FertilizerPrice }

export default function FertilizerPricesEdit({ item }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        country: item.country, fertilizer_key: item.fertilizer_key,
        min_price: String(item.min_price), max_price: String(item.max_price),
        price_per_bag: String(item.price_per_bag), price_active: item.price_active,
        sort_order: String(item.sort_order ?? ''), desc: item.desc ?? '',
    })

    function handleSubmit(e: FormEvent) { e.preventDefault(); put(`/admin/fertilizer-prices/${item.id}`) }

    return (
        <AdminLayout title="Edit Fertilizer Price">
            <div className="mx-auto" style={{ maxWidth: 640 }}>
                <ResourceForm title="Edit Fertilizer Price" onSubmit={handleSubmit} processing={processing} onCancel={() => window.history.back()}>
